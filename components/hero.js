...
const Hero = () => {
  ...
  const settings = {
    ...
    speed: 500,
    slidesToShow:
      posts?.CompanyLogo?.length <= 5
        ? posts.CompanyLogo.length
        : posts?.CompanyLogo?.length - 1,
    autoplay: true,
    cssEase: "ease-in-out", // Set the CSS easing property
    arrows: false,
    pauseOnHover: false,
    responsive: [
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: Math.min(4, posts?.CompanyLogo?.length || 0),
        },
      },
      {
        breakpoint: 768,
        settings: {
          slidesToShow: Math.min(3, posts?.CompanyLogo?.length || 0),
        },
      },
      {
        breakpoint: 480,
        settings: {
          slidesToShow: Math.min(2, posts?.CompanyLogo?.length || 0),
        },
      },
    ],
  };
      ...
      <Container>
        <div className="flex flex-col justify-center">
          <div className="text-xl text-center text-gray-700 dark:text-white">
            Trusted by{" "}
            <span className="text-indigo-600">{posts.TotalCustomer}+</span>{" "}
            customers worldwide
          </div>
          {/* Added CompanyLogo in loop */}
          {/* <div className="flex flex-wrap justify-center gap-5 mt-10 md:justify-around">
            {posts.CompanyLogo && (
              <Slider {...settings} className="w-full" arrows={false}>
                {posts.CompanyLogo.map((logo, index) => (
                  <div
                    key={index}
          ...
          <div className="flex flex-wrap justify-center gap-5 mt-10 md:justify-around">
            {posts?.CompanyLogo && posts.CompanyLogo.length > 0 ? (
